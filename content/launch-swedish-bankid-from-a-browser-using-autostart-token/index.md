...
[taxonomies]
tags = ["bankid", "frontend", "integration"]
...
First, we use the User-Agent string to identify the user's operating system and browser. This will be used for the [returnUrl](@/launch-swedish-bankid-from-a-browser-using-autostart-token/index.md#returnurl)-value and selecting which [URI-scheme](@/launch-swedish-bankid-from-a-browser-using-autostart-token/index.md#uri-schemes) to use.
...
<https://app.bankid.com/> blocks iframes with the http header *[X-Frame-Options](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/X-Frame-Options): DENY*, as well as their [content security policy](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) configuration of *default-src 'self'*.
